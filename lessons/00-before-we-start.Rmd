...
author: Dan McGlinn and other Data Carpentry contributors 
...
* Under the `Files` tab on the right of the screen, click on `New Folder` and
  create a folder named `scripts` within your newly created working directory.
  (e.g., `~/quant_methods/scripts`)
* Create a new R script (File > New File > R script) and save it in your scripts
  directory (e.g. `./scripts/data-carpentry-script.R`)

Your working directory should now look like this:

![How it should look like at the beginning of this lesson](figures/r_starting_how_it_should_look.png)
...
A good reference for the different components of the Rstudio IDE is this reference card: 
...
forgotten when you close the session (Note they will be saved in Rstudio's
history panel but this isn't ideal). It is better to enter the commands in the
...
* Additional [motivations](../motivations)
...
be read as 3 **goes into** `x`.  You can also use `=` for assignments. Most R 
users prefer to use the `<-` operator but I personally prefer `=` because it has 
one less keystroke and is more asthesticly pleasing. The choice is ultimately
up to the user. 
...
Rstudio provides quick argument documentation using the tab key. Simply write a
function's name with the leading the parentheses and hit the `Tab` key to get a
list of arguments and a short description of each one.
...
There is an extensive list of R cheatsheets and reference cards. 
Here is just a short list of useful ones: 

* [base-r.pdf](http://github.com/rstudio/cheatsheets/raw/master/base-r.pdf)
* [short card](https://cran.r-project.org/doc/contrib/Short-refcard.pdf)
