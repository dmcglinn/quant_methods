...
* <a href="#tab">Tabular Model Interpretation
...
which model is the best? One approach is to use the principle of parsimony 
...
library(MASS)             # for stepAIC to automate model selection 
library(marginaleffects)  # to plot model predictions
```
...
```{r}
# the traditional way to plot the fitted model predictions is to use the 
# function predict 
...
More recently the `marginaleffects` package has make it easier to generate plots
of fitted models, for example:

```{r}
plot_predictions(main_mod, condition = c('plant_ht_cm', 'trt'), points = 1) +
  xlab("Plant height (cm)") + 
  ylab("Fruit mass (mg)") 
```

As you can see the two approaches produce similar output but the newer approach
is more efficient. 

Another way to think about the main effect model is that it is specifying a 3d
...
Or more efficiently we can simply use: 
```{r}
plot_predictions(int_mod, condition = c('plant_ht_cm', 'trt'), points = 1) +
  xlab('Plant height (cm)') + 
  ylab('Fruit mass (mg)')
```
...
## <a href="#tab" name="tab">#</a> Tabular model interpretation
