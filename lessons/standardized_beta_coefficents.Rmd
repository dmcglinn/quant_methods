...
We can use the function `scale()` to standardize the independent and
dependent variables. 
...
Notice that above the t- and p-values between `mod` and `mod_std` don't change
(with the exception of the intercept term which is always 0 in a regression of
standaridized variables.
